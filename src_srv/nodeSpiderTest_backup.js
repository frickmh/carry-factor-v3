    ...
    var callback = function(url, i) {
      ...
      request( {url: url, reqUrl: url, index: i}, function(error, response, body) {
            ...
            if (currentMatch - 1 < endMatch && response.statusCode != 429) {
              ...
              output.gameId = this.index;
              output.responseCode = response.statusCode;

              output.gameCreation = "NULL";

              for (var j = 0; j < 10; j++) {


                    output["summonerId" + j] = "NULL";
                    output["champId" + j] = "NULL";
                    output["kills" + j] = "NULL";
                    output["deaths" + j] = "NULL";
                    output["assists" + j] = "NULL";
                    output["win" + j] = "NULL";

              }

              output.isPlatinum = "NULL";
...
//      currentMatch += 1;
    ...
    var fixErrors = function() {
      console.log("Errors will be fixed here!");

      //sql = "SELECT * FROM `matches` WHERE responseCode >= 400;";
      sqlFix = "SELECT gameId FROM `matches` WHERE responseCode >= 399;";
